...
**技术实现**:
- 前端使用React + react-dnd或react-beautiful-dnd实现拖拽功能
- 组件配置JSON格式存储
- 实时预览功能
- 使用React Hooks管理组件状态
...
- **算法类型**:
  - 负载均衡算法: 根据数据集当前负载选择
  - 时间分区算法: 根据查询时间范围选择对应分区数据集
  - 业务规则算法: 根据业务字段值匹配规则选择数据集
  - 性能优先算法: 根据数据集响应速度选择
...
POST /api/data/select-dataset
{
    ...
    "algorithm": "time_partition"  # 或 "load_balance", "business_rule", "performance"
}
```
...
- **TypeScript**: 推荐使用TypeScript进行类型安全开发
